...
function ModalMyDivar({
  ...
  useOutsideClick(modalRef, openMyDivar, setOpenMyDivar);
  return (
    <div
      ref={modalRef}
      className="transition-none absolute top-[44px] -left-[125.016px] bg-white rounded-[4px] cursor-pointer flex flex-col outline-none overflow-hidden z-50 shadow-lg  "
    >
      <div className="w-[224px] shadow-[0_1px_6px_0_rgba(0,0,0,0.08),0_-8px_32px_-4px_rgba(0,0,0,0.04),0_16px_24px_-6px_rgba(0,0,0,0.04)]">
        ...
        {isLoggedIn ? (
          <Link
            to={"/dashboard"}
            className="flex items-center text-[rgba(0,0,0,0.56)] cursor-pointer h-12 outline-none px-4 transition-colors duration-[36s] ease-[ease] w-full space-x-2"
          >
            <SVGLogin
              width={"15px"}
              height={"15px"}
            ...
            />
            <p className=" text-xs font-normal m-0 leading-0.5 ">حساب کاربری</p>
          </Link>
        ) : (
          <button
